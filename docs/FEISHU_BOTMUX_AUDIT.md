...
| Direct/group/topic routing and mention handling | Adapt | Implemented; private chat defaults to one topic and Session binding per top-level message, with an optional flat continuous-chat mode. Only `root_id` + `thread_id` is treated as a real private topic reply. Group topic lookup fails closed, and both `open_id` and `app_id` bot mentions are distinguished from mentions of other users/apps. `always`/`topic`/`bound` group trigger modes are per bot. |
