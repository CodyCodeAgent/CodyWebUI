# Codex Web Local 最终产品目标

本文档定义 `codex-web-local` 的长期目标。

最终目标不是简单把 Codex 桌面版搬到浏览器里，而是把它做成一个本地优先、浏览器可控、可并行、可审计、可回滚的 AI 软件工程操作系统。用户应该可以在任意设备上启动、监督、审批、审查、验证、回滚和交付 Codex 编程任务。

## 一句话定位

`codex-web-local` 应该演进成一个 **Codex Agent Control Tower**。
...
`codex-web-local` 应该让用户像操作一个工程系统一样操作 Codex：任务化、多 Agent、安全、可审查、可验证、可回放、可回滚、支持移动端，并且支持视觉皮肤和布局自定义。

最短路径是先把可信远程控制层做好。最雄心勃勃的终局，是一个可自定义的 Codex Agent Control Tower，用于严肃的本地和远程软件工程工作。
